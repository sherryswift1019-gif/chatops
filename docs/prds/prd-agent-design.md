...
**Web chat「继续对话」的 PRD 上下文注入**:

PRD 管理页点击「继续对话」会创建一条绑定 `prdId` 的 `prd_chat_sessions` 记录（[web/src/pages/PrdDocumentsPage.tsx](../web/src/pages/PrdDocumentsPage.tsx) `handleContinueChat`）。

为了避免 Claude 把用户后续的"审查 / 改一下 / 看看"等指代解析为随机搜到的其他 PRD，stream 路由会把 `session.prdId` 透传给 `streamWebChat`，后者用 `buildPrdContext(prdId)` 读出当前 PRD 的 id / 标题 / 版本 / 状态，追加到 systemPrompt 末尾，并明确告诫 Claude「不要调 `search_existing_prds` 去找别的 PRD，也不要自行猜 prdId」。链路：

```
handleContinueChat(prd)
  └─ POST /prd-chat/sessions { productLineId, prdId }
       └─ createChatSession 持久化 prdId 到 DB
            └─ SSE stream 路由读回 session.prdId
                 └─ runner.streamWebChat({ ..., prdId })
                      └─ buildPrdContext(prdId) 拼到 systemPrompt
```

这是**新建 PRD 对话**（`prd_id=null`）和**继续已有 PRD 对话**（`prd_id=X`）两种入口共享同一个 `streamWebChat` 的分叉点——有 prdId 就锁上下文，没有就按常规 Phase 1 起步。
...
findings.some(f => f.severity === 'blocker')
    | yes
    | findings 中所有 blocker 的 canAutoFix 均为 true
    |     -> [Agent 自修复] 把 fixable blocker 反馈给生成 Agent
    ...
    | 任一 blocker 的 canAutoFix = false（通常 ownership=pm/business）
    |     -> 直接 review_blocked，升级人工
    | no blocker
    -> 状态 -> draft，交付（附 findings 供参考）
```

**修复触发规则**（[src/agent/prd/prd-agent.ts](../src/agent/prd/prd-agent.ts) `runPrdReview`）：
- 进入修复分支的唯一判据是 **blocker.canAutoFix**：blocker 全部 `canAutoFix=true` 才跑 REPAIR；任一为 false 直接升级人工
- `ownership` 仅用于 Web 审批面板的归属标签显示（`需 PM 补充` / `需管理员补充` / `业务决策`），**不参与修复门控**——REVIEW agent 的 `canAutoFix` 自评已经隐含地把"需新事实/需决策"的条目筛出
- `warning` / `info` 级别的 finding **不触发修复**，只记录在 `reviewResult.findings`，由管理员在 Web 端自行决定是否处理
- 修复后的 Markdown 必须通过 `validatePrdStructure` 结构校验才会写回 DB；不合格则回滚并升级人工
- 最多 `MAX_REPAIR_ROUNDS = 2` 轮修复，避免死循环
...
reviewer 在 PRD 详情点「驳回 PM」后：

1. 状态从 `review_blocked` 回到 `drafting`；
2. `prd_documents.review_history` 末条记录里存了 `recommendation.reason`（"人工驳回: {comment}（by {decidedBy}）"）；
3. 但 PM 完全没感知 —— 列表没有「我的 PRD」筛选、详情没有驳回原因提示、也没有入口回到对话继续修改。

一期（§17 之前）只做了「驳回」动作本身，没做 PM 侧的承接。本节补齐闭环。

### 18.2 设计目标

**列表筛选 → 详情提示 → 一键回到对话（带驳回上下文）→ 同时发 DingTalk DM 告知 PM**。

四件套同步落地，缺一不可：

| 环节 | 目的 |
|------|------|
| 「我的 PRD」筛选 | PM 登录后能一眼看到自己待处理的 PRD |
| 详情页 Alert banner | 把 review_history 里的驳回原因显式前置 |
| 「回到对话修改」按钮 | 一键新建 chat session，带驳回上下文 |
| DingTalk DM 通知 | 用户离开 web 后也能收到触达 |

### 18.3 前端改动

#### 18.3.1 「我的 PRD」筛选

- 页面：`web/src/pages/PrdDocumentsPage.tsx`
- 当前用户名从 `AdminLayout` 下传（已调用 `/admin/auth/me`）
- 过滤栏加 `Segmented`「全部 / 我的 PRD」，默认「全部」
- 切到「我的 PRD」时 `load()` 附加 `createdBy: currentUsername`
- 复用已有后端 `created_by` 查询参数（`src/admin/routes/prd-documents.ts` GET `/prd-documents` 已支持）

#### 18.3.2 驳回 Alert + 回到对话按钮

触发条件：`prd.status === 'drafting'` 且 `reviewHistory.at(-1)?.result?.recommendation?.action === 'reject'`。
...
<Alert
  type="warning"
  showIcon
  message="该 PRD 已被驳回，请按驳回原因修改后重新提交"
  description={
    <div>
      <div><b>驳回原因</b>：{lastReview.result.recommendation.reason}</div>
      {/* 可选：展示上一轮 AI 自审 blockers 摘要（从倒数第二条 review_history 拿） */}
    </div>
  }
  action={
    <Button type="primary" onClick={handleResumeChat}>
      回到对话修改
    </Button>
  }
/>
...
```ts
const { data } = await createPrdChatSession({
  productLineId: prd.productLineId,
  prdId: prd.id,
  seedRejection: true,
})
navigate(`/prd-chat/${data.sessionKey}`)
```

### 18.4 后端改动

#### 18.4.1 POST `/admin/prd-chat/sessions` 支持 `seed_rejection`

- 入参：`{ product_line_id, prd_id?, seed_rejection?: boolean }`
- 当 `seed_rejection === true && prd_id != null`：
  1. `getPrdDocumentById(prd_id)` 拿最新 `reviewHistory`
  2. 末条若 `action === 'reject'` → 拼 seed 文本（标题 + 版本 + 驳回原因 + 上一轮 blockers）
  3. `appendChatMessage({ sessionKey, role: 'assistant', content: seedText, metadata: { kind: 'reject_seed' } })`
- PM 打开会话第一眼即见驳回上下文。

**Seed 文本格式示例**：

```
⚠️ 本次对话基于 PRD #42《订单导出》的最近一次驳回意见继续。

**驳回原因**：人工驳回: 登录态描述不清楚（by alice）

**上一轮自审 blockers**（如还没修可以先讨论这些）：
- [structure/high] 未声明 SSO 回调 URL 规则
- [functional/high] 批量导出权限分级缺失

我可以帮你梳理修改方向，告诉我想先聊哪一条。
```

注意：manual reject 时 `findings = []`（见 `src/admin/routes/prd-documents.ts:163`），blockers 需从倒数第二条 review_history（上一轮 AI 自审 `review_blocked` 时的记录）回溯；拿不到就省略该段。

#### 18.4.2 系统提示注入驳回摘要

在 `claude-runner.ts` 构建 PRD 系统提示的位置（`buildPrdContext` 或等价函数）：

- 若 PRD 处于 `drafting` 且最近一次 review `action === 'reject'` → 在系统提示里追加：
...
这样无论 chat session 是带 `seed_rejection` 新建的，还是 resume 旧的，Claude 下一轮都能看到驳回上下文 —— 不依赖 UI seed 消息。

#### 18.4.3 DingTalk DM 通知（best-effort）

位置：`src/admin/routes/prd-documents.ts` reject 分支，`appendReviewHistory` 之后。

流程：

```ts
// 1. 尝试用 createdBy 反查钉钉用户（IM 创建的 PRD，createdBy 就是钉钉 staffId）
const dtUser = await getDingTalkUserById(prd.createdBy)
if (!dtUser) {
  // web 后台创建、无钉钉映射 —— 只 log，不阻塞
  logger.info({ prdId, createdBy }, 'skip DM: not a dingtalk user')
} else {
  // 2. 通过 adapter 发 DM（仿照 src/approval/gate.ts:157-160）
  const text = [
    `📝 你的 PRD「${prd.title}」被驳回了`,
    ``,
    `原因：${reason}`,
    WEB_BASE_URL ? `` : null,
    WEB_BASE_URL ? `查看详情：${WEB_BASE_URL}/prd-documents/${prd.id}` : null,
  ].filter(Boolean).join('\n')
  
  await notifyDm(dtUser.userId, text).catch((e) => logger.warn(e, 'DM failed'))
}
```

接入方式：仿照 `src/server.ts` 里现有 `setNotifyDmFn` 模式，给 `registerPrdDocumentRoutes` 注入 `notifyDm` 函数，避免引入新的全局状态。

**兜底约定**：
- `WEB_BASE_URL` 未配置时省略链接行，其余内容照发；
- DM 失败只 log，不影响 `review-decision` 返回 200。
...
生产部署设 `WEB_BASE_URL=https://chatops.paraview.cn`。

### 18.6 验证方式

**手工 E2E（DingTalk 路径）**：

1. 钉钉群 @bot 创建 PRD（`created_by` = 钉钉 staffId）
2. 自审落到 `review_blocked`
3. web 后台详情点「驳回 PM」，填写 comment
4. 预期：
   - PRD 状态回到 `drafting`
   - PM 的钉钉收到私聊（标题 + 原因 + 链接）
   - 列表「我的 PRD」切开能看到
   - 详情 Alert 展示驳回原因
   - 点「回到对话修改」 → 跳新 chat session，首条消息是 seed
   - 发一句试修改，Claude 能引用驳回点（系统提示注入生效）

**纯 web 路径（无钉钉映射）**：
- Web 后台创建 PRD + 驳回：DM 不发（log `skip DM, ... not in dingtalk_users`），三件套 UI 正常工作。

**单元覆盖**：
- `buildRejectSeedText()` 纯函数 spec：覆盖「无 blockers / 有 blockers / 无 reject entry」三种情况
- `pnpm tsc --noEmit` 零报错

**回归面**：
- 现有 `POST /prd-chat/sessions`（不带 seed_rejection）行为不变
- reject 分支任何 DM 失败都不能影响 200 返回
- 「我的 PRD」关闭时列表不漏数据

### 18.7 不在本期范围

- Feishu DM 通路（一期只钉钉；Feishu adapter 也有 `sendDirectMessage`，后续直接拷）
- PRD 详情页修改历史 timeline（目前 review_history 只在 drawer 展开）
- 给 web 后台用户补「钉钉 staffId 映射」字段（工作量大，靠 IM 路径自然对齐）
