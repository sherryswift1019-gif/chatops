...
| 9 | 全局超时 | 7 天，可配（`bug_reports_timeout_days`） | §14 全局超时 |
...
在 `system_config` 表（假设存在，否则新建）新增：

| key | 默认值 | 含义 |
|-----|--------|------|
| `bug_reports_timeout_days` | 7 | 全局 Bug 实例超时（决策 9） |
| `revise_max_rounds` | 3 | revise 轮次上限（决策 1） |
| `owner_response_timeout_seconds` | 暂不设 | 主备 owner 响应超时；决策 11 选 FCFS 不需要升级，此项留位未用 |
...
-- 6. system_config 新增 bug_reports_timeout_days / revise_max_rounds
...
VALUES
  ('bug_reports_timeout_days', '7', 'Bug 实例全局超时天数（超过 → aborted）'),
...
export async function handleMrClosed(mrIid: number, reportId: number, closeComment: string) {
  ...
  const maxRounds = Number(await getConfig('revise_max_rounds') ?? 3)
...
const maxRevise = Number(await getConfig('revise_max_rounds') ?? 3)
...
export function startTimeoutScheduler() {
  cron.schedule('7 * * * *', async () => {  // 每小时第 7 分钟（不与 analyzer 并发碰撞）
    const timeoutDays = Number(await getConfig('bug_reports_timeout_days') ?? 7)
...
`bug_reports_timeout_days` 在 `system_config` 表中可改，改后下次 cron 生效（无需重启）。值为 0 表示禁用超时。
...
export async function runPreflight(): Promise<PreflightResult> {
  ...
  const limit = Number(process.env.BUG_INFLIGHT_LIMIT ?? 10)
  if (inflight >= limit) {
...
- **Given** 配置 `bug_reports_timeout_days=0`
...
阶段 A：数据层 + MCP 前置（可并行做）
  ...
  ├─ A3  system_config repository + config 读取工具
...
阶段 H：上线与观察
  ...
  └─ H3  根据数据决定是否需要调整参数（revise_max_rounds / 超时天数 等）
```
...
| `src/db/repositories/system-config.ts` | system_config 表 repository |
...
- [ ] 补齐 `system_config` 初始值（`bug_reports_timeout_days=7`, `revise_max_rounds=3`）
