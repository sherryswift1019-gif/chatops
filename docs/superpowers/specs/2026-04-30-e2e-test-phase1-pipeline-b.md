# 端到端自动化测试 — Phase 1 · Pipeline B：Test-and-Fix Loop

> 立项日期：2026-04-30
> 范围：**仅 Phase 1 中的 Pipeline B**（起沙盒 → 跑测试 → 失败收证据 → e2e-fix llm_agent 修复 → 重部署 → 重跑直至全绿 → 开 MR）
> 姊妹文档：
> - `docs/superpowers/specs/2026-04-30-e2e-test-phase1-pipeline-a.md`（Pipeline A：Test Generator）
...
> §0-2（目的 / 总览 / 数据模型 / 脚本契约）与 Pipeline A 文档共享，方便单独阅读本文档；§4 Pipeline B 是本文档主体；§3 Pipeline A 不在本文档，详见姊妹文档。Pipeline B 假设被测项目要么有现成 e2e 脚本（`tests/e2e/*.spec.ts`），要么 Pipeline A 已经把 markdown spec 生成的脚本 auto-merge 进 main。
...
> Pipeline A 相关的 `/e2e-specs` 管理页 / 模板 / lint 入口见姊妹文档 `pipeline-a.md` 的 §5.3 / §5.8。
...
| **周 4（Pipeline B 主线）** | Pipeline B 图 + bugfix 同步 await + e2e_run capability + IM 单句 | • Pipeline B 集成测过（mock 所有 invoke_target_script，跑完整图，验状态转移 + governor + reset_iteration_branch）<br>• `e2e_run` capability + IM 单句语法 `@bot 跑 chatops e2e [--tag=...]`<br>• 命令行能触发完整 run（不带 UI）<br>• 进程 kill 后重启，inflight run 被 recovery 标 aborted |
| **周 5（验收 + UI）** | 简化 UI（runs 列表 + 详情页 polling）+ 简单合 MR + dogfood 验收 | • `/e2e-runs` 列表页 + `/e2e-runs/:runId` 详情页（场景时间线 + evidence 查看 + polling 5s 刷新，**不做 SSE**）<br>• `/e2e-targets` 只读详情页（chatops 一行硬编码）<br>• 「中止」按钮触发清理顺序（标 status + teardown sandbox）<br>• Pipeline B 全绿后**开普通 MR**给人审（不做 PR 描述模板 / cherry-pick）<br>• **dogfood 验收**：IM 触发 e2e_run（输入是 Pipeline A 已经 merge 进 main 的 .spec.ts）→ 故意改坏一个产品文件 → 自动诊断 → 修复 → 重跑通过；中间故意 kill chatops 进程 → 重启后 run 被 recovery；从 dogfood 期手工统计：(a) Pipeline B 修复成功率（人 review 后判定）；(b) AI 误报率；(c) 节省人时（用户填问卷）|

**Pipeline B done 的标志**：
1. dogfood 验收完整跑通 + 至少修复一个真实产品 bug 自动合 MR（人审通过）
2. 进程 kill 重启后 inflight run 被 recovery 正确标 aborted + sandbox teardown
3. 手工统计 3 个 ROI 指标（修复成功率 / 误报率 / 节省人时）作为后续投入决策依据

> 周 3（Pipeline A 完整 + `/e2e-specs` 页）见姊妹文档 `pipeline-a.md`。Pipeline B 周 5 dogfood 的输入（`.spec.ts` 文件）来自 Pipeline A 在周 3 完成的 auto-merge。
