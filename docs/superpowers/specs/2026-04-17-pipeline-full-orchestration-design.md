...
由 `notify_bug` handler 在 Pipeline 的最后 stage 执行（通知内容本次硬编码，模板配置记入 [Growth Backlog](../../docs/product/growth-backlog-notification-template.md)）：

| 场景 | 通知谁 | 内容 |
|------|--------|------|
| L1/L2/L3 全部成功 | 各 project 负责人 + 触发人 | "Bug 已自动修复，MR !{mrIid} 等待合并" + MR 链接 + Review 结论 |
| AI Review 需关注 | 各 project 负责人 | "AI Review 发现问题，请关注 MR !{mrIid}" + MR 链接 |
| L4 | 触发人 | "已创建 Issue #{issueIid}，此问题需要人工处理" + Issue 链接 + 分析摘要 |
| 修复失败（L2/L3 fix 阶段失败到顶） | 触发人 | "修复失败，需要人工介入" + Issue 链接 + 失败原因 |
| 审批被拒 | 触发人 | "L3 修复方案审批被拒绝" |
| 审批超时 | 触发人 | "L3 审批超时，Pipeline 已终止，可在 Bug 修复实例页面重试" |
...
- 触发人不论多少 project，只发一条汇总通知（列出全部 MR 链接）
...
- **Then**：
  ...
  - notify_bug 发成功通知：每个 project owner 只看到自己 project 的 MR；触发人看到 2 个 MR 汇总
