...
7. 仅触发人场景（l4_created / approval_rejected / approval_timeout / approval_retry_analysis / fix_failed）：handler 直接返回 success，不发 DM 也不写 notify 事件（本版不做触发人 DM，前端通过 Bug 修复实例页面的 status 和事件时间线展示）
...
| L4 / 修复失败 / 审批被拒 / 审批超时 / retry_analysis | 不发 DM | 前端通过 `bug_analysis_reports.status` 和事件时间线展示，管理后台可手动重试 |
