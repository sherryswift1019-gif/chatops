...
### 3.6 Pipeline A 出 PR 的人审 SLA 与 default reviewer

Pipeline A 创建的 PR **必须配置默认审阅人 + SLA 提醒**，避免 spec 永远卡 `pr_open`。

| 配置项 | 第一期默认 |
|---|---|
| `e2e_target_projects.capabilities.prDefaultReviewers` | 项目 owner（从 `internal_capability_pipelines` 反查）|
| `e2e_target_projects.capabilities.prSlaHours` | 48 小时 |
| 超时行为 | IM 提醒（第一次 24h、第二次 48h 升到群主审）|

cron 扫 `WHERE generation_status='pr_open' AND last_generated_at < NOW() - INTERVAL '48 hours'` 触发 IM 提醒。dogfood 期 ChatOps 自身 default reviewer 就是项目 owner。

### 3.7 第一期范围

**做**：平台兜底 generator / static_check / baseline self-correct / commit + PR + default reviewer + SLA 提醒 / 单 spec 触发
...
| **周 5** | Pipeline A + 规约页 + IM 完整路径 + dogfood 验收 | • Pipeline A 单 spec 跑通（生成 → static check → baseline self-correct → commit + 出 PR + 配 default reviewer）<br>• `/e2e-specs` 管理页（含 baseline 失败 evidence 查看）<br>• IM 完整路径（启动 / 中间通知 / 完成）<br>• **dogfood 验收**：手写 2 spec → A 生成 → B 跑通；故意改坏一个产品文件 → 自动诊断 → 修复 → 重跑通过；中间故意 kill chatops 进程 → 重启后 run 被 recovery |
...
第一期不做分布式 tracing。等 V2 ssh-proxy 接入、多模块产品落地后引入 OpenTelemetry，把 setup_sandbox / deploy / run_scenario / dispatch_bugfix 各 stage 串成 trace。

### 6.6 后续阶段路线图（仅记录）

- **第二期**：ssh-proxy 接入（验证多项目 contract）+ scenario 并行 + 失败留沙盒 + `affected_modules` 增量 redeploy + e2e-targets 完整 CRUD + ping-pong 检测
