...
本次实施**仅交付 Phase 1（MVP 闭环，5 周）**。Phase 2-7 内容在 spec 主体内仍可见，作为未来路线图占位 + 架构口子说明，但**不在本期实施范围**。

| Phase | 重点 | 工期 | 状态 |
|---|---|---|---|
| **Phase 1 — MVP 闭环** | 让"测试发现 bug → 自动修复 → 重跑 → 全绿"端到端跑通；dogfood ChatOps 自身；**包含完整 Pipeline A（AI 从 markdown 生成 Playwright 脚本 + baseline self-correct + auto-merge）**——测试脚本准确度是整个闭环的根基，self-correct 是区分"脚本 bug vs 产品 bug"的工程必要项 | **5 周** | **🟢 本 spec 实施范围** |
| Phase 2 — 测试脚本质量增强 | TestSpec 模板 + lint / Playwright codegen 录制入口（D6，第二种 spec 来源）/ 业务指标完整（4 个 + `e2e_value_estimates` 表）| 2-3 周 | 🔮 未来 |
| Phase 3 — MR 集成 + 生产化 | MR webhook 主入口 + 汇总 MR 完整形态 + Auto-verify + 一键 revert + 角色权限 | 3 周 | 🔮 未来 |
| Phase 4 — 易用性 + 完整 UI | SSE 实时刷新 + 完整通知策略 + Run abort 完整清理流程 | 2 周 | 🔮 未来 |
...
- **TestSpec 模板 + lint** → Phase 2。第一期人手写 markdown spec，没有模板辅助 / lint 节点
- **Playwright codegen 录制入口（第二种 spec 来源）** → Phase 4
...
| 测试脚本来源 | **AI 从 markdown spec 生成 Playwright（Pipeline A 完整版）**：包含 baseline self-correct（区分脚本 bug vs 产品 bug）+ auto-merge | P1 |
| TestSpec 模板 / lint / codegen 录制 | 第一期人手写 markdown，无模板 / lint；codegen 录制入口推 P4 | P2 / P4 |
...
dogfood 时给 ChatOps 项目登记两条映射；外部项目接入时（Phase 5）再追加。
...
**Phase 2+ schema 演进**：
- Phase 2 `schema-v1001.sql`：`e2e_value_estimates` 表（业务指标）+ TestSpec 模板 / lint 配置
...
> **Phase 1 必含** — 测试脚本准确度是整个闭环的根基。如果脚本错了，下游"自动诊断 + 自动修复"会被污染（误诊为产品 bug → bugfix 修了不该修的代码）。**baseline self-correct 是工程必要项**，是区分"脚本 bug vs 产品 bug"的唯一可行办法。
> 
> Phase 1 包含：generator + static_check + baseline self-correct + commit + auto-merge + `e2e_specs` 表 + `/e2e-specs` 管理页 + `e2e_generate_script` capability。
> 
> Phase 1 **不含**（推到 Phase 2）：TestSpec 模板 + lint 节点、Playwright codegen 录制入口（D6，第二种 spec 来源）、业务指标完整。
...
> 🔮 **Phase 2** — Phase 1 人手写 markdown spec，无模板辅助 / lint 节点。本节是 Phase 2 设计稿占位。
...
> 🔮 **Phase 4** — Phase 1 只支持人写 markdown spec → Pipeline A 生成。codegen 录制入口作为第二种 spec 来源推到 Phase 4。本节是 Phase 4 设计稿占位。
...
**不做**（推后）：
- 多 spec 并行（max_concurrency=1）→ Phase 2+
- content_hash 自动触发 → Phase 2+
- 项目侧 `--generate` 实现入口 → Phase 5（外部项目接入时按需）
...
3. 手工统计的 4 个 ROI 指标作为 Phase 2 立项依据
...
| Phase | 重点 | 工期 | 关键产出 |
|---|---|---|---|
| **P1 — MVP 闭环** ✅本 spec | 让"测试发现 bug → 自动修复 → 重跑 → 全绿"端到端跑通；**含完整 Pipeline A（baseline self-correct + auto-merge）** | 5 周 | Pipeline A 完整 + Pipeline B 完整 / `e2e_specs` 表 / `/e2e-specs` 页面 / IM 单句触发 / 简化 UI / 简单合 MR / dogfood 验收 + ROI 数据 |
| **P2 — 测试脚本质量增强** | TestSpec 模板 + lint / 业务指标完整 | 2-3 周 | TestSpec 模板（金标 spec 例子）/ spec lint 节点 / `e2e_value_estimates` 表 / verify-run-passed 表单 / 业务指标完整 4 个 |
