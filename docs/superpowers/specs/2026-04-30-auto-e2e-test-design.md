...
> 立项日期：2026-04-30
> 状态：设计稿（**Phase 1 MVP**），待审
> 关联：现有 bugfix 流水线、PRD 流水线、Pipeline 引擎（LangGraph）

---

## ⚠️ 本 spec 当前覆盖范围 — Phase 1 MVP

本次实施**仅交付 Phase 1（MVP 闭环，4 周）**。Phase 2-7 内容在 spec 主体内仍可见，作为未来路线图占位 + 架构口子说明，但**不在本期实施范围**。

| Phase | 重点 | 工期 | 状态 |
|---|---|---|---|
| **Phase 1 — MVP 闭环** | 让"测试发现 bug → 自动修复 → 重跑 → 全绿"端到端跑通；dogfood ChatOps 自身；人手写 Playwright 脚本（不做 generator） | **4 周** | **🟢 本 spec 实施范围** |
| Phase 2 — 测试脚本自动化 | Pipeline A（markdown → Playwright 脚本生成）+ TestSpec 模板 + 业务指标完整 | 3 周 | 🔮 未来 |
| Phase 3 — MR 集成 + 生产化 | MR webhook 主入口 + 汇总 MR 完整形态 + Auto-verify + 一键 revert + 角色权限 | 3 周 | 🔮 未来 |
| Phase 4 — 易用性 + 完整 UI | Playwright codegen 录制入口 + SSE 实时刷新 + 完整通知策略 | 2 周 | 🔮 未来 |
| Phase 5 — 多项目接入 | ssh-proxy 接入 + e2e-targets CRUD + 跨项目 dashboard + lint 工具 + 上线推广 | 3 周 | 🔮 未来 |
| Phase 6 — 多模块多机环境编排 | k8s / VM pool / remote-multi-host（独立立项需求）| 6+ 周 | 🔮 未来 |
| Phase 7+ — 高级特性 | bugfix 独立 worker / wait_webhook / max_concurrency>1 / ping-pong 检测 / PRD→Spec | 按需 | 🔮 未来 |

每节内容如果属于 Phase 2+，会在节首打 **🔮 Phase X**  标注。Phase 1 主体不带标注。
...
### Phase 1 不在本期范围（推到 Phase 2+）

- **Pipeline A（测试脚本自动生成）** → Phase 2。第一期人手写 Playwright 脚本
- **MR webhook 主入口 + comment / status check 回 MR** → Phase 3。第一期 IM 单句触发为唯一入口
- **汇总 MR 完整形态**（PR 描述模板 + 逐 commit 接受/拒绝 + cherry-pick + `e2e_summary_mr_decisions` 表）→ Phase 3。第一期全绿后开一个普通 MR 给人审
- **Auto-verify on merge / 一键 revert** → Phase 3
- **角色 / 权限矩阵** → Phase 3。第一期 platform admin 一锅端
- **Playwright codegen 录制入口** → Phase 4
- **SSE 实时刷新 + 详情页 drawer 渲染各 mimeType** → Phase 4。第一期详情页 polling 兜底，drawer 渲染基础
- **业务指标完整**（4 个业务指标 + `e2e_value_estimates` 表）→ Phase 2。第一期只采集基础工程指标
- **TestSpec 模板 + lint** → Phase 2
- **多项目接入**（ssh-proxy 等）+ e2e-targets 完整 CRUD + 跨项目 dashboard + 上线推广 → Phase 5
- **多模块多机环境编排**（k8s / VM pool / remote-multi-host）→ Phase 6（独立立项）
- **PRD → Spec 自动派生** → Phase 7+

### 关键决策汇总

| 决策点 | 选择 | Phase |
|---|---|---|
| 被测对象 | 第一期 ChatOps 自身；架构支持外部项目接入 | P1 / P5 |
| 失败回路范式 | 同步闭环：一次 run 跑到全过 | P1 |
| 被测环境形态 | 临时沙盒（每次 run 起一份隔离环境）| P1 |
| 沙盒底座 | docker-compose-local（k8s / VM pool 推到 P6）| P1 |
| 测试脚本来源 | **人手写 Playwright**（位于 `tests/e2e/*.spec.ts`）；从 markdown 生成推到 P2；codegen 录制推到 P4 | P1 |
| 浏览器驱动 | Playwright + Playwright MCP（AI 探索时通过 MCP）| P1 |
| Bugfix 应用方式 | 沙盒内 iteration_branch 直接 commit；全绿后开普通 MR 给人审（汇总 MR 复杂形态推到 P3）| P1 |
| 项目接入抽象 | shell 脚本契约（4 个约定脚本：build.sh / deploy.sh / test.sh / 可选 fix.sh）；平台一边只有一个通用节点 + 一张登记表 | P1 |
| Pipeline B 主入口 | **IM 单句**`@bot 跑 chatops e2e [--tag=smoke]`；MR webhook 主入口推到 P3 | P1 |
| Pipeline B 循环 | LangGraph 原生 conditional_edge 自循环 | P1 |
| Bugfix 分发 | 同进程同步 await（独立 worker / webhook 推到 P7+）| P1 |
| Schema 编号策略 | 本模块从 v1000 起步独立段位，跟主干 v59+ 永不撞号 | P1 |
...
> 🔮 **Phase 2** — 第一期不创建此表。**Phase 1 schema-v1000.sql 不包含 `e2e_specs`**。本表 + 关联状态机是 Phase 2 设计稿占位。
...
- **Phase 1**: `e2e_run` → 映射到 Pipeline B（test-and-fix loop）
- 🔮 Phase 2: `e2e_generate_script` → 映射到 Pipeline A（generator + baseline self-correct）

dogfood 时给 ChatOps 项目登记 `e2e_run`；外部项目接入时（Phase 5）再追加。
...
- `CREATE TABLE` **3 张新表**（不含 `e2e_specs`，那是 Phase 2）：
  - `e2e_target_projects`
  - `e2e_runs`
  - `e2e_scenario_runs`
  - `e2e_sandboxes`
- **硬编码 `INSERT INTO e2e_target_projects`** 一行 chatops 项目（dogfood）
- `ALTER TABLE bug_analysis_reports` 加 3 列（`triggering_e2e_run_id` / `triggering_e2e_scenario_run_id` / `sandbox_branch`；`affected_modules` 推迟到 V2）
- `INSERT INTO pipeline_node_types` 1 个新节点（`invoke_target_script`；不新增 `loop`）
- `INSERT INTO internal_capability_pipelines` 1 条映射（`e2e_run` → Pipeline B；`e2e_generate_script` 推到 Phase 2）
- **末尾追加** `pipeline_node_types.enabled` 行数 ≥ 13 的硬断言（跟 schema-v35 同形式）

**Phase 2+ schema 演进**：
- Phase 2 `schema-v1001.sql`：`e2e_specs` 表 + `e2e_value_estimates` 表（业务指标）+ `e2e_generate_script` capability 映射
- Phase 3 `schema-v1002.sql`：`e2e_summary_mr_decisions` 表
- 后续按需继续 v1003+
...
> 🔮 **Phase 2** — 第一期不实现。第一期人手写 Playwright 脚本放进 `tests/e2e/*.spec.ts`，Pipeline B 直接调 `test.sh --discover` 现读，不走 generator。本节是 Phase 2 的设计稿占位，第一期完成后会重新评估（包括是否真的需要、是否改用 Phase 4 codegen 录制为主路径）。
> 
> **Phase 1 不创建 `e2e_specs` 表 / `e2e_generate_script` capability / `/e2e-specs` 页面**。schema-v1000.sql 不包含 `e2e_specs` 建表。
...
### 3.9 Pipeline A 范围（Phase 2 时再细化）

> 🔮 整个 §3 已经标 Phase 2，下表仅作 Phase 2 立项时参考。第一期人手写 Playwright 脚本到 `tests/e2e/*.spec.ts`，不需要 Pipeline A。
...
> ⚠️ **Phase 1 触发只走 IM 单句 + UI**（API/scheduled 预留接口不主推）。MR webhook 主入口推到 **Phase 3**。下面"主入口 GitLab MR webhook 自动触发"部分是 Phase 3 设计稿占位，第一期不实现。

1. ~~**🥇 主入口 — GitLab MR webhook 自动触发**~~ → 🔮 **Phase 3 推迟**：MR 创建 / 更新（push commit）时自动触发一次 e2e_run，结果以 comment + status check 形式回 MR。这是 dev 工作流的自然集成点，但第一期不实现 — 用 IM 触发顶上
2. **🥇 第一期主入口 — IM 单句命令**：`@bot 跑 chatops e2e [--tag=smoke]` 单句语法（D4：默认全跑，`--tag` / `--id` 参数可选）
3. **🥈 后台 UI** — `/e2e-runs` 列表的「新建 Run」按钮。复杂参数（governor 覆盖）/ 调试场景
4. **API / scheduled** — 第一期不主推；预留接口

#### MR webhook 触发协议

> 🔮 **Phase 3 设计稿占位**。第一期不实现。
...
- 自动 close 临时 e2e-auto issue（基础版，§4.8）
- IM 通知关键节点
...
- scenario 并行 → P7+
- ping-pong 检测 → P7+
- `affected_modules` redeploy → P6
- `wait_webhook + HTTP callback` 协议 → P7+
- 失败留沙盒 → P7+
- **汇总 MR 复杂形态**（§4.9）→ P3
- **Auto-verify on merge / 一键 revert**（§4.10）→ P3
- **完整角色权限矩阵**（§4.11）→ P3
...
> 🔮 **Phase 3** — 第一期不实现。第一期 iteration_branch 全绿后**开一个普通 MR**（标准 GitLab MR 流程，简单 commit list + 人审），不做 PR 描述模板 / 逐 commit 接受拒绝 / cherry-pick / `e2e_summary_mr_decisions` 表。本节是 Phase 3 的设计稿占位。
...
> 🔮 **Phase 3** — 第一期不实现。dogfood 期 ChatOps 自身团队人少，由 reviewer 人审兜底；万一漏过坏 commit 走标准 GitLab `git revert` 流程。本节是 Phase 3 的设计稿占位。
...
> 🔮 **Phase 3** — 第一期不实现完整 RBAC。dogfood 期 platform admin / 项目 owner 一锅端（任何登录用户都能触发 e2e_run / abort），后续接入外部项目前必须把这一节落地。本节是 Phase 3 的设计稿占位。
...
> 🔮 **Phase 2** — 第一期不实现。第一期人手在 GitLab 上 commit Playwright 脚本到 `tests/e2e/*.spec.ts`，不需要 spec 管理页。本节是 Phase 2 的设计稿占位。
...
> 🔮 **Phase 4** — SSE 实时刷新推到 Phase 4。**第一期详情页用 polling 兜底**（5s 间隔轮询 `/admin/e2e-runs/:runId` 更新页面）。Phase 4 完整 SSE 上线后无缝切换。下方 §5.5.1 SSE schema 是 Phase 4 设计稿占位。
...
**不做**（推后）：
- `/e2e-specs` 管理页 → P2
- SSE 实时刷新 + 详情页 drawer 渲染各 mimeType → P4
- Run abort 完整 4 步清理（第一期只做"标 status + teardown sandbox"两步）→ P4
- 跨项目聚合 / dashboard → P5
- in-page markdown 编辑器 → P2
- e2e-targets 完整 CRUD → P5
- 「Lint 接入」按钮 → P5
...
| **周 3** | Pipeline B 图 + bugfix 同步 await + e2e_run capability + IM 单句 | • Pipeline B 集成测过（mock 所有 invoke_target_script，跑完整图，验状态转移 + governor + reset_iteration_branch）<br>• `e2e_run` capability + IM 单句语法 `@bot 跑 chatops e2e [--tag=...]`<br>• 命令行能触发完整 run（不带 UI）<br>• 进程 kill 后重启，inflight run 被 recovery 标 aborted |
| **周 4** | 简化 UI（runs 列表 + 详情页 polling）+ 简单合 MR + dogfood 验收 | • 列表页 + 详情页基础（场景时间线 + evidence 查看 + polling 5s 刷新，**不做 SSE**）<br>• `/e2e-targets` 只读详情页（chatops 一行硬编码）<br>• 「中止」按钮触发清理顺序<br>• Pipeline B 全绿后**开普通 MR**给人审（不做 PR 描述模板 / cherry-pick）<br>• **dogfood 验收**：手写 1-2 个 Playwright 测试放进 `tests/e2e/` → IM 触发 e2e_run → 故意改坏一个产品文件 → 自动诊断 → 修复 → 重跑通过；中间故意 kill chatops 进程 → 重启后 run 被 recovery；从 dogfood 期手工统计：(a) 修复成功率（人 review 后判定）；(b) AI 误报率；(c) 节省人时（用户填问卷）|

**Phase 1 done 的标志**：
1. dogfood 验收完整跑通 + 至少修复一个真实产品 bug 自动合 MR（人审通过）
2. 手工统计的 3 个 ROI 指标作为 Phase 2 立项依据

---

### 6.5 ~~指标与日志（可观察性）~~

仅保留 §6.5.2 工程指标 + §6.5.3 structured log 字段约定。业务指标（§6.5.1）推到 Phase 2。

### 6.5 指标与日志（可观察性）

#### 6.5.1 业务指标（P2 — 决策者拍板继续投资的依据）

> 🔮 **Phase 2** — 业务指标完整采集（4 个指标 + `e2e_value_estimates` 表 + verify-run-passed 表单）推到 Phase 2。**Phase 1 只采集 §6.5.2 的工程指标**，dogfood 4 周后通过手工统计（人审 review、问卷收集）拿初步 ROI 数据，作为 Phase 2 立项依据。本节是 Phase 2 设计稿占位。
...
> 🔮 **Phase 5** — 第一期不需要。dogfood 阶段只在 ChatOps 团队内部使用，不需要接入手册 / demo 视频 / sample 仓库 / 失败案例库。Phase 5 多项目接入前必须落地。本节是 Phase 5 设计稿占位。
...
```
1. 项目方在自己仓库根目录有 / 扩展 4 个脚本（参考 chatops 自己的实现）：
     build.sh / deploy.sh / test.sh （+ 可选 fix.sh）
...
4. 项目方在自己仓库 .gitlab-ci.yml 加一个步骤跑 chatops-test-target lint，
   验证 4 个脚本子命令完整 + stdout JSON 协议合法（V2 提供这个 lint 工具）
...
### 6.8 跨项目 dashboard（P9）

> 🔮 **Phase 5** — 第一期单项目（chatops）不需要跨项目 dashboard。Phase 5 多项目接入时一起做。本节是 Phase 5 设计稿占位。
...
按"价值递进 + 风险可控 + 依赖顺序"分期。每期独立交付价值，每期完成后重新评估下一期是否值得投入。

| Phase | 重点 | 工期 | 关键产出 |
|---|---|---|---|
| **P1 — MVP 闭环** ✅本 spec | 让"测试发现 bug → 自动修复 → 重跑 → 全绿"端到端跑通 | 4 周 | Pipeline B 完整 / IM 单句触发 / 简化 UI / 简单合 MR / dogfood 验收 + ROI 数据 |
| **P2 — 测试脚本自动化** | Pipeline A：从 markdown 生成 Playwright 脚本 + baseline self-correct | 3 周 | `e2e_specs` 表 / Pipeline A / TestSpec 模板 + lint / 业务指标完整（4 个 + `e2e_value_estimates`）/ `/e2e-specs` 页面 |
| **P3 — MR 集成 + 生产化** | 从"想起来用"工具 → "MR 自然集成" | 3 周 | MR webhook 主入口 + comment / status check 回 MR / 汇总 MR 完整形态（PR 描述模板 + 逐 commit 接受拒绝 + cherry-pick + `e2e_summary_mr_decisions` 表）/ Auto-verify on merge / 一键 revert / 角色权限矩阵 |
| **P4 — 易用性 + 完整 UI** | 提升 dogfood + 接入用户体验 | 2 周 | Playwright codegen 录制入口 / SSE 实时刷新 + 详情页 drawer 渲染 / 完整通知策略 / Run abort 完整清理流程 |
| **P5 — 多项目接入** | 验证 contract 通用性 + 推广能力 | 3 周 | ssh-proxy 接入（Go CLI 项目）/ `e2e-targets` 完整 CRUD / 跨项目 dashboard 基础 / `chatops-test-target lint` CLI 工具 / 上线推广方案落地（接入手册 + demo + sample 仓库）/ 失败案例库整理 |
| **P6 — 多模块多机环境编排** | 独立立项需求；本模块自动受益 | 6+ 周 | k8s namespace / VM pool / remote-multi-host 沙盒实现 / `affected_modules` 增量 redeploy / scenario 并行 / 资源池调度 |
| **P7+ — 高级特性** | 按需 | TBD | bugfix 跑独立 worker → `wait_webhook + HTTP callback + HMAC` 协议 / `max_concurrency > 1` / Ping-pong 检测 / 失败留沙盒（人手调试）/ PRD → Spec 自动派生 / 法律合规审计 |

#### P1 → P2 启动判断

P1 dogfood 完成后**手工统计**这 3 个数据，决定 P2 是否立项：

| 指标 | 启动 P2 阈值 |
|---|---|
| AI 修复成功率（dogfood 期人 review 后判定）| ≥ 30%（修不好的还能由人接手）|
| AI 误报率（被误诊为产品 bug 的脚本 bug）| ≤ 30% |
| 节省人时（用户问卷收集，每个修复成功 case 估算）| 总和 ≥ 5 小时（4 周内）|

任一不达标 → 不立项 P2，反思 spec 调整方向（比如砍掉 Pipeline A 改主推 codegen 录制；或彻底 sunset 见 §6.9）。

#### P2 → P3 启动判断

P2 完成后看：
- Pipeline A 自动生成 + auto-merge 的稳定性（baseline_failed 率 ≤ 20%）
- 业务指标完整数据：4 周 dogfood 累计 ≥ 30 个 e2e run（statistical significance）
