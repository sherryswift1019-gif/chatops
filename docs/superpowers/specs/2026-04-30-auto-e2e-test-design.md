...
| Phase | 重点 | 工期 | 状态 |
|---|---|---|---|
| **Phase 1 — MVP 闭环** | 让"测试发现 bug → 自动修复 → 重跑 → 全绿"端到端跑通；dogfood ChatOps 自身；**包含完整 Pipeline A（AI 从 markdown 生成 Playwright 脚本 + baseline self-correct + auto-merge）+ MVP 用独立 e2e-fix llm_agent 节点替代 bugfix capability（永远不接，A 决策）** | **5 周** | **🟢 本 spec 实施范围** |
...
| Bugfix 应用方式 | **MVP 用独立 e2e-fix llm_agent 节点 + 精简 debug-fix prompt**（不调 bugfix capability，永远不接）；沙盒内 iteration_branch 直接 commit；全绿后开普通 MR 给人审 | P1 永远独立 |
...
> **MVP 决策已废止**：原计划 e2e 失败走 `analyze_bug` capability 自动建 GitLab issue，需要单独管理 issue label 和 close 流程。MVP 改用 e2e-fix llm_agent 节点（§4.3）后**完全不创建 GitLab issue**，诊断结果直接落 `e2e_scenario_runs.evidence_manifest.aiDiagnosis` JSONB 字段。本节作废。
