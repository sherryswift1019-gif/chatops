...
```
外部系统
  │ POST /admin/api/webhooks/pipeline/:token   Body: { ...JSON, "_servers"? }
  ▼
...
```
POST /admin/api/webhooks/pipeline/:token
...
每次 **token 命中 + payload 合法**的请求（无论 pipeline 后续成败）写一条 `audit_log`：

```json
{
  "category": "webhook-trigger",
  "actor": "webhook:<id>:<name>",
  "target": "pipeline:<pid>",
  "detail": {
    "webhookId": 123,
    ...
    "runId": 456,
    "payloadSize": 8421,
    "ipHash": "sha256(remoteAddr)[0:16]"
  }
}
```

- payload 本身不写 audit（已经在 `test_runs.trigger_params` 完整保存）
- 401 / 429 不写（防探测刷爆）
- 404（认证后）写
- 5xx 写
...
| 场景 | 时机 | HTTP | 写 test_runs? | 写 audit? |
|------|------|------|--------------|----------|
| body 不是 JSON / 不是 object | 同步 | 400 | 否 | 否 |
| body > 1MB | Fastify pre-parse | 413 | 否 | 否 |
| token 不存在 / 禁用 | 同步 | 401 | 否 | 否 |
| 限流 | 同步 | 429 + Retry-After | 否 | 否 |
| pipeline 不存在 / 禁用 | 同步 | 404 | 否 | 是 |
| `_servers` 字段类型错 | 同步 | 400 | 否 | 是 |
| `_servers` 引用未知 server | 异步 | 202（已发） | 是（failed + error_message） | 是 |
| 制品输入解析失败 | 异步 | 202（已发） | 是 | 是 |
| stage 失败 | 异步 | 202（已发） | 是 | 是 |
| webhook-router 自身崩溃 | 同步 | 500 | 否 | 是 |

原则：401 / 429 不写 audit（防探测刷爆）；其他通过认证的请求都写。
...
未来若要加任何一项，对应 ALTER TABLE 即可，不需要重构。
