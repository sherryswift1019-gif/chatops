...
> 约定：TypeScript 类型与文件引用写作 `EnrichedInput`（PascalCase）；JSON 实例与字段路径写作 `enrichedInput`（camelCase）。

[src/quick-impl/enriched-input-schema.ts](../../src/quick-impl/enriched-input-schema.ts) 中的 `EnrichedInput` Zod schema 是三方共识的数据合同。任何 role 都不应自行重定义这些字段的含义。brainstorm-host 的职责是尽量填满所有字段；当 `partial: true` 时，`missingFields` 明确列出哪些字段没有足够信息，spec-author 必须将这些字段对应的问题列为 `OPEN_QUESTION`，不允许以默认值掩盖信息缺口。
...
[scripts/qi-spec-lint.ts](../../scripts/qi-spec-lint.ts) 承担所有 mechanical 校验，三方 role 不重复这些规则，只做人工/语义层判断：
...
AC Then 子句含主观词 → 见 §3 反模式黑名单（主观词列表）
