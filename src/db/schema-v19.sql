-- ============================================================
-- schema-v19: Bind a default pipeline to a capability (IM-triggered)
-- 原本命名为 schema-v13；upstream main 先占用 v13/v14/v15（handover/analysis 扩展）、
-- 后又占用 v16/v17（PRD Agent），合并 main 时第三次挪号到 v19。
-- ============================================================
--
...
BEGIN
  ...
  IF v_pl_id IS NULL THEN
    RAISE NOTICE 'schema-v19 seed: no product_lines, skipping deploy-im-demo';
    RETURN;
  END IF;
  ...
  IF v_pipeline_id IS NULL THEN
    ...
    RAISE NOTICE 'schema-v19 seed: deploy-im-demo pipeline created id=%', v_pipeline_id;
  ELSE
    RAISE NOTICE 'schema-v19 seed: deploy-im-demo pipeline already exists id=%', v_pipeline_id;
  END IF;
