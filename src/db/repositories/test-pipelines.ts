...
export async function updateTestPipeline(id: number, data: Partial<{
  ...
  const { rows } = await pool.query(
    ...
    [id, data.name ?? null, data.description ?? null,
     ...
     data.containerImage ?? null,
     'paramSchema' in data, data.paramSchema !== undefined ? (data.paramSchema === null ? null : JSON.stringify(data.paramSchema)) : null,
     'imPrompt' in data, data.imPrompt !== undefined ? data.imPrompt : null]
  )
  return rows[0] ? mapRow(rows[0]) : null
}
