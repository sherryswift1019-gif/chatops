...
describe('schema-v1012 retry_counters extension', () => {
  beforeEach(async () => { await resetTestDb() })

  it('v1012 migration is registered in _migrations table', async () => {
    const pool = getTestPool()
    const { rows } = await pool.query<{ version: string }>(
      `SELECT version FROM _migrations WHERE version='v1012'`,
    )
