...
describe('im-triggers repository', () => {
  ...
  it('createIMTrigger / updateIMTrigger / deleteIMTrigger round-trip', async () => {
    ...
    expect(created.capabilityKey).toBeNull()
  ...
  it('createIMTrigger with capabilityKey stores and retrieves correctly', async () => {
    // 找一个存在的 capability key
    const { listCapabilities } = await import('../../db/repositories/capabilities.js')
    const caps = await listCapabilities()
    if (caps.length === 0) return
    const capKey = caps[0].key

    const created = await createIMTrigger({
      key: 'test_trigger_capkey',
      displayName: '能力关联测试',
      description: 'capabilityKey unit test',
      pipelineId: null,
      capabilityKey: capKey,
      isSystem: false,
      enabled: true,
    })
    expect(created.capabilityKey).toBe(capKey)
    expect(created.pipelineId).toBeNull()

    // update: 换回 null
    const updated = await updateIMTrigger(created.id, { capabilityKey: null })
    expect(updated!.capabilityKey).toBeNull()
  ...
  it('v43 migration: im_triggers with matching capability have capabilityKey set', async () => {
    const all = await listIMTriggers()
    // v43 migration 确保：有匹配 capability 且无 pipeline 的触发器都已关联 capabilityKey
    for (const t of all) {
      expect(t.capabilityKey === null || typeof t.capabilityKey === 'string').toBe(true)
    }
  })
})
