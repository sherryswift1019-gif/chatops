/**
 * QI v12 graph 结构 + e2e fix-loop 子机集成测试
 *
 * 不依赖真实 DB / Claude / Playwright — 验证 v12 graph 的关键结构性不变量：
 *   1. 所有必需节点存在（qi_e2e_runner / im_input / dev_fix_author / 各 router）
 ...
 *   5. QUICK_IMPL_TEMPLATE_VERSION = 12
 *
...
async function loadV12Graph(): Promise<Graph> {
  capturedGraph = null
...
describe('QI v12 graph 结构 + fix-loop 子机', () => {
  it('QUICK_IMPL_TEMPLATE_VERSION = 12', () => {
    expect(bootstrap.QUICK_IMPL_TEMPLATE_VERSION).toBe(12)
  })

  it('必需节点全部存在', async () => {
    const g = await loadV12Graph()
  ...
  it('旧节点（v9）不再出现在 v12 graph', async () => {
    const g = await loadV12Graph()
  ...
  it('线性主链：init → spec_author → spec_ai_review → spec_human_gate → spec_commit_push → plan_author', async () => {
    const g = await loadV12Graph()
    const linearChain = ['init_branch', 'spec_author', 'spec_ai_review', 'spec_human_gate', 'spec_commit_push', 'plan_author']
  ...
  it('dev_push → qi_e2e_runner → e2e_router 连通', async () => {
    const g = await loadV12Graph()
    expect(g.edges.some((e) => e.source === 'dev_push' && e.target === 'qi_e2e_runner')).toBe(true)
