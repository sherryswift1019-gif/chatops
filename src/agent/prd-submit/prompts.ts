...
- **blocker**：缺失核心要素（目标、指标、验收、数据模型等），或度量方式明显不可执行。有 blocker → decision=blocked。
...
{"decision":"pass","findings":[{"severity":"warning","title":"成功指标缺度量方式","detail":"§1.3 的 '触达率 ≥ 98%' 没有说明统计周期和样本范围，建议补充"}],"markdown":"## 🤖 AI Review 结论\\n\\n**结论**: ✅ pass\\n\\n**主要发现**:\\n- [warning] 成功指标缺度量方式：§1.3 的 '触达率 ≥ 98%' 没有说明统计周期和样本范围\\n\\n建议在下一轮迭代补充，本次 MR 可合并。"}

## 硬约束（违反会被拒绝）

1. 只输出一段 JSON，不要 \`\`\`json 围栏，不要前后文
...
6. 不要调用任何工具（Read/Glob/Grep），只看 diff 做判断（MVP 不做跨文档一致性校验）
`
